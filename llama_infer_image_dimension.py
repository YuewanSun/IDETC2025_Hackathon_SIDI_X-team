...
INPUT_CSV = r"rule_functional_performance_qa_with_predictions_with_context.csv"       # must have columns: prompt, image
...
def parse_response(response: str) -> tuple[str, str]:
    """
    Parse the model response to extract Yes/No answer and explanation.
    Returns (prediction, explanation) tuple.
    """
    response = response.strip()
    
    # Try to find Yes/No at the beginning
    yes_no_pattern = r'^(Yes|No|YES|NO|yes|no)\b'
    match = re.search(yes_no_pattern, response)
    
    if match:
        prediction = match.group(1).capitalize()  # Normalize to "Yes" or "No"
        # Get the explanation part (everything after the Yes/No)
        explanation = response[match.end():].strip()
        # Remove common separators at the beginning of explanation
        explanation = re.sub(r'^[,.\-:\s]+', '', explanation)
    else:
        # If no clear Yes/No found, try to extract from context
        if any(word in response.lower() for word in ['yes', 'correct', 'compliant', 'meets']):
            prediction = "Yes"
        elif any(word in response.lower() for word in ['no', 'incorrect', 'non-compliant', 'violates', 'does not meet']):
            prediction = "No"
        else:
            prediction = "Unclear"
...
image_folder = r"C:\Users\ys25268\OneDrive - The University of Texas at Austin\Windows\Reaserch2\IDECT_hackathon\design_qa\dataset\rule_compliance\rule_functional_performance_qa\images"
# Updated user constraint for clearer parsing
user_constrain = "Answer with 'Yes' or 'No' first, then provide a one sentence explanation. Format: Yes/No, [explanation]"
...
for i, row in tqdm(df.iterrows(), total=len(df), desc="Inferring"):
    ...
    try:
        ...
        # Parse the response into prediction and explanation
        prediction, explanation = parse_response(response)
        ...
        print(f"Error for row {i}: {error_msg}")

# Save results with two separate columns
df["model_prediction"] = predictions
df["explanation"] = explanations
df.to_csv(OUTPUT_CSV, index=False)
print(f"✅ Done. Saved results to: {OUTPUT_CSV}")
print(f"Processed {len(df)} rows. Results saved with 'model_prediction' and 'explanation' columns.")